...
pub fn generate(out_dir: &str, shell: Option<CompletionShell>) -> Result<(), String> {
    ...
    let mut cmd_a = rift_wm::cli::rift::Cli::command();
    println!("Generating completions for rift to {out_dir}...");
    let mut cmd_b = rift_wm::cli::rift_cli::Cli::command();
    println!("Generating completions for rift_cli to {out_dir}...");

    if let Some(shell) = shell {
        generate_single(shell, "rift", &mut cmd_a, gen_dir)?;
        generate_single(shell, "rift-cli", &mut cmd_b, gen_dir)?;
...
fn generate_single(
    ...
    match shell {
        CompletionShell::Bash => generate_to(clap_complete::Shell::Bash, cmd, binary_name, out_dir)
        ...
        CompletionShell::Fish => generate_to(clap_complete::Shell::Fish, cmd, binary_name, out_dir)
        ...
        CompletionShell::Zsh => generate_to(clap_complete::Shell::Zsh, cmd, binary_name, out_dir)
